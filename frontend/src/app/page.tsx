...
export default function UsersPage() {
  ...
  const handleSaveUser = async (id: number) => {
    ...
    await fetch(`${API}/users/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: userToUpdate.name, email: userToUpdate.email }),
    });
    alert("Usuário salvo!");
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, id: number, field: 'name' | 'email') => {
  ...
  return (
    <main className='flex min-h-screen flex-col items-center p-8 md:p-12 lg:p-24'>
      ...
      <p className='mb-8'>Status API: <strong className='text-green-600'>{status}</strong></p>

      <Button variant="secondary" className='button-primary' onClick={() => router.push('cadastro')}>ir para cadastro Página</Button>
